...
def show_current_config(config: Dict[str, Any]):
    ...
    print(f"   Sample Repeat: {training.get('sample_repeat', 4)}x")
...
def configure_training(config: Dict[str, Any]):
    ...
    training['sample_repeat'] = get_input("Sample repeat (each sample shown N times)", training.get('sample_repeat', 4), int)
