...
_video_debug_counter = [0]  # Mutable to track across calls
...
def train_video_diffusion_step(video_generator, video_frames, text_context, target_size=256, sample_types=None):
    ...
    """
    _video_debug_counter[0] += 1
    debug_this = _video_debug_counter[0] <= 5  # Debug first 5 calls
    
    if video_generator is None:
        if debug_this:
            print(f"      [DEBUG] video_generator is None")
        return None
    if video_frames is None:
        if debug_this:
            print(f"      [DEBUG] video_frames is None")
        return None
    try:
        if not isinstance(video_frames, torch.Tensor):
            if debug_this:
                print(f"      [DEBUG] video_frames not tensor: {type(video_frames)}")
            return None
        if video_frames.numel() == 0:
            if debug_this:
                print(f"      [DEBUG] video_frames empty")
            return None
        ...
        if debug_this:
            print(f"      [DEBUG] video_frames shape: {video_frames.shape}, sample_types: {sample_types[:3] if sample_types else None}...")
        ...
        if sample_types is not None:
            ...
            if not type_mask.any():
                if debug_this:
                    print(f"      [DEBUG] no video sample types in batch")
                return None
        ...
        if video_frames.dim() == 5:
            ...
            if debug_this:
                print(f"      [DEBUG] video_frames wrong dim: {video_frames.dim()}")
            return None
        ...
        if debug_this:
            print(f"      [DEBUG] valid videos: {num_valid}/{B}, frame_sum_range: {video_frames.abs().sum(dim=(1,2,3,4)).min().item():.2f}-{video_frames.abs().sum(dim=(1,2,3,4)).max().item():.2f}")
        if not valid_mask.any():
            if debug_this:
                print(f"      [DEBUG] all video frames are zeros!")
            return None
